...
13. Switch vulnerable dockerfile to basic, and add the SBOM generation, upload and evaluation steps. [Link to this commit](https://github.com/nilsoriano/shiftleft/commit/634209f6753df715b54df0f2f7f09e3023cb6de0)
14. SBOM vuln eval doesn't pass all checks, because it detects a non-fixed HIGH vuln in c-ares. The only way I can think of to make Dockerfilebasic pass all tests, would be to change this step's `--fail-on` flag from high to critical, but this would not match the "fail build on HIGH or CRITICAL vulns" requisite found in other steps of this exercise. After doing this, Dockerfilebasic passes all checks, as seen in this [job run](https://github.com/nilsoriano/shiftleft/actions/runs/19964213443/job/57251721257)
